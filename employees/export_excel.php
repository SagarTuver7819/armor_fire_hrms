...
$headers = [
    ...
    'Shift Type', 'Shift Time', 'PF Deduction', 'PF Start Date', 'Employee Contribution', 'Employer Contribution', 'UAN',
...
if ($result) {
    while ($row = $result->fetch_assoc()) {
        $cells = [
            ...
            $row['shift_time'] ?? '',
            $row['pf_deduction'] ?? '',
            formatDateDisplay($row['pf_start_date'] ?? ''),
            $row['pf_employee_contribution'] ?? '',
            $row['pf_employer_contribution'] ?? '',
            $row['uan_number'] ?? '',
            $row['bank_name'] ?? '',
