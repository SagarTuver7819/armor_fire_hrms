...
$row = $id > 0 ? getCircularById($id) : null;
if ($id > 0 && !$row) {
...
$formError = '';
$formData = [];
if (!empty($_SESSION['circular_form']) && is_array($_SESSION['circular_form'])) {
    $formError = (string) ($_SESSION['circular_form']['error'] ?? '');
    $formData = is_array($_SESSION['circular_form']['data'] ?? null)
        ? $_SESSION['circular_form']['data']
        : [];
...
<main class="dashboard-main">
    ...
    <div class="form-page-card">
        ...
        <form method="POST"
              action="<?php echo app_url('circulars/save.php'); ?>"
              class="circular-form"
            ...
            <div class="form-section">
                <div class="form-grid form-grid-3">
                    <div class="form-group span-2">
                        <label for="circularTitle">Title <span class="req">*</span></label>
                        <input type="text" name="circular_title" id="circularTitle" class="form-control" required maxlength="255"
                               placeholder="e.g. Leave Policy Update / Office Timing"
                               value="<?php echo htmlspecialchars($val('title')); ?>">
                    </div>
                    <div class="form-group">
                        <label for="circularNo">Circular No</label>
                        <input type="text" name="circular_no" id="circularNo" class="form-control" maxlength="100"
                               placeholder="Optional ref no."
                               value="<?php echo htmlspecialchars($val('circular_no')); ?>">
                    </div>
                    <div class="form-group">
                        <label for="circularDate">Circular Date <span class="req">*</span> <small>(DD-MM-YYYY)</small></label>
                        <input type="text" name="circular_date" id="circularDate" class="form-control js-date" required
                               placeholder="DD-MM-YYYY"
                               value="<?php echo htmlspecialchars(dateInputValue($val('circular_date', date('Y-m-d')))); ?>">
                        <small class="form-hint">Date printed / written on the circular</small>
                    </div>
                    <div class="form-group">
                        ...
                        <small class="form-hint">Date this circular was entered in HRMS</small>
                    </div>
                    <div class="form-group">
                        <label for="circularPdf">
                            Scan PDF (Hard Copy)
                            <?php if (!$row): ?><span class="req">*</span><?php endif; ?>
                        </label>
                        <input type="file" name="circular_pdf" id="circularPdf" class="form-control"
                    ...
                    <div class="form-group full">
                        <label for="circularRemarks">Remarks</label>
                        <textarea name="remarks" id="circularRemarks" class="form-control" rows="3"
            ...
            <div class="form-actions sticky-actions">
                <a href="<?php echo app_url('circulars/index.php'); ?>" class="btn-secondary">Cancel</a>
                <button type="submit" class="btn-primary" id="btnSaveCircular">
                    <i class="fa-solid fa-floppy-disk"></i>
                    <?php echo $row ? 'Update Circular' : 'Save Circular'; ?>
                </button>
            </div>
        </form>
    </div>
