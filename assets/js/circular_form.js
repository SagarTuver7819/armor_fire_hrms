/**
 * Circular form â€” department multi-select (Select2 + checkboxes)
...
(function ($) {
    ...
    function initCircularDeptSelect() {
        var $sel = $('#circDeptSelect');
        var form = document.getElementById('circularForm');
        if (!$sel.length || !form) {
            return;
        }
        ...
        try {
            $sel.select2({
                width: '100%',
                placeholder: $sel.data('placeholder') || 'Search & select departmentsâ€¦',
                allowClear: false,
                closeOnSelect: false,
        ...
        $sel.off('change.circDept select2:select.circDept select2:unselect.circDept')
            .on('change.circDept select2:select.circDept select2:unselect.circDept', function (ev) {
                var vals = currentVals();
                if (vals.length) {
        ...
        $('#circSelectAllBtn, #circSelectAllLink').off('click.circDept').on('click.circDept', function (e) {
            e.preventDefault();
            setAll();
            $sel.select2('close');
        });
        $('#circClearDeptBtn').off('click.circDept').on('click.circDept', function (e) {
            e.preventDefault();
            clearAll();
        });
        ...
        $(form).off('submit.circDept').on('submit.circDept', function (e) {
            var title = document.getElementById('circularTitle');
            var pdf = document.getElementById('circularPdf');
            if (title) {
